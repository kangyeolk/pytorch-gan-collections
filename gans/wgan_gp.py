    ...
    def train(self):
        ...
        for epoch in range(self.args.epoch):
            ...
            for it, (x, _) in enumerate(self.data_loader):
                ...
                # clipping D (wgan) => gradient penalty (wgan-gp)
                alpha = torch.rand((x.size(0), 1, 1, 1)).to(self.args.device)
                x_hat = alpha * x.data + (1 - alpha) * fake_x.data
                x_hat.requires_grad = True
                
                pred_hat = self.D(x_hat)
                grad_outputs = torch.ones(pred_hat.size()).to(self.args.device)
                ...
                gradient_penalty = self.lambda_gp *((gradients.view(gradients.shape[0], -1).norm(p=2, dim=1) - 1) ** 2).mean()
                ...
                loss_dict['D/gradient_penalty'] = gradient_penalty.item()
